...
import {getSituation,setInfSituation} from "../../utils/APIRoutes";
import { InputNumber } from "primereact/inputnumber";
import { Button } from "primereact/button"
import { Toast } from 'primereact/toast';
export default function Situation (props)
{
  const[situationdetail,setsituationdetail]=useState({ id_asf: props.idasf})
  ...
  const headerStyle = {
    ...
    return(    
    <Dialog  headerStyle={{ backgroundColor: 'var(--green-400)',color:'#fff',borderRadius:'20px', height:"4rem",marginBottom:'5px'}} header="Situation" visible={true} style={{ width: '80vw' }} onHide={() => props.setsituation(false)}>
        <Toast ref={toast} position="top-center"  />
       <Accordion multiple activeIndex={[0,1]} style={{ color: "#fff", marginTop: "1rem" }}>
       <AccordionTab style={{ backgroundColor: 'var(--green-400)' }} header="Détail de la situation">
       <div className="flex flex-wrap gap-3 p-fluid pr-3">
        ...
        <AccordionTab style={{ backgroundColor: 'var(--green-400)' }} header="Situation">
